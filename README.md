...
But before you can use the plugin, you need to tell it where its info file will go so it can check what's changed. Since this file doesn't have anything sensitive in it, it's easiest just to download it over HTTP(S) from where you'll be hosting this website. For example:
```js
// ...
...
I'd suggest having your do both though, depending on if it's a test build or not. It's up to you or your template to decide exactly how you want to do that, but I do it like this in [my template](https://github.com/hedgehog125/SvelteKit-Template):
```js
import { sveltekit } from "@sveltejs/kit/vite";
...
# Options
 * **Required**: **`lastInfo`**: function returning Promise<string> or string
 
   Normally you can import and use fetchLast or readLast, but read on if you want to make a custom one. This function should return a promise or immediately return the last Versioned Worker info file (unparsed). If the file doesn't exist, the promise can resolve to null or null can be immediately returned, this will create an initial info file in the build.
 ...
 * **`lazyCache`**: function returning boolean or Promise<Boolean> (default: **function that returns false**)
   ...
   But otherwise, this function determines if file of the build should be lazy cached or not (true means it should, false means it shouldn't). It runs at build time, rather than when the client is downloading files. A lazy cached file isn't downloaded ahead of time, so your program should be able to handle it not being there. It gets downloaded when it's requested, unless it can't because the device is offline.
  ...
  * **`exclude`**: function returning boolean or Promise<Boolean> (default: **function that excludes a few unnecessary files**)

    Works in the same way as lazyCache and is called with the same arguments. Return true or resolve to true to exclude, return false to include. Note that lazyCache doesn't wait for the result of this function, although it will run synchronously first. lazyCache will still be called even if this function synchronously excludes that file though.
    
  * **`generateManifest`**: function returning an object or string, or a promise for either (default: **function that makes some slight tweaks to how it works**)
  ...
  * **`handlerFile`**: string (default: **"src/hooks.worker.js"**)
    ...
    Where the plugin should look for your handler file. This file doesn't have to exist. Relative to the root of your SvelteKit project. More information about it in the next section.
...
You can hook into and override the default behaviour of the worker at runtime. You do this by exporting functions from your `handlerFile`, which is looked for at "src/hooks.worker.js" by default. Currently, there's only 1 hook that's supported:
  ...
  * **`handle`**: function returning Promise<Response>, Promise<null> or immediately returning either
  
    The function is called with the path without the base URL or starting slash, if it's a page or not, the fetch event and the full path. If the promise resolves to a Response object, it'll be sent as the response, but if null, the default behaviour will happen instead (both responses and nulls can also be directly returned).
